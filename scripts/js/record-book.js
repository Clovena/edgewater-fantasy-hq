...
async function loadStandingsData() {
  try {
    const response = await fetch('/data/sixth-city/api/v_fact_standings.csv');
...
function populateSeasonFilter() {
  // Get unique seasons
  const seasons = [
    ...new Set(standingsData.map((row) => parseInt(row.season))),
  ].sort((a, b) => b - a);
...
function updateSeasonLabel() {
  ...
  if (checkedCount === 0) {
    ...
    label.textContent = `${sortedSeasons[0]}, ${sortedSeasons[1]}${
      checkedCount > 2 ? ', ...' : ''
    }`;
  }
}
...
function aggregateStandings() {
  ...
  filteredData.forEach((row) => {
    ...
    franchise.wins += parseFloat(row.wins) || 0;
    franchise.losses += parseFloat(row.losses) || 0;
    franchise.points_for += parseFloat(row.points_for) || 0;
    franchise.points_against += parseFloat(row.points_against) || 0;
    franchise.max_points_for += parseFloat(row.max_points_for) || 0;
  });

  // Calculate win percentage and sort
  const aggregated = Array.from(franchiseMap.values()).map((franchise) => {
    const totalGames = franchise.wins + franchise.losses;
    const totalPointsSq = franchise.max_points_for ^ 2;
    const PointsAgainstSq = franchise.points_against ^ 2;
    franchise.win_pct = totalGames > 0 ? franchise.wins / totalGames : 0;
    franchise.accuracy =
      franchise.max_points_for > 0
        ? franchise.points_for / franchise.max_points_for
        : 0;
    return franchise;
  });
  ...
  aggregated.sort((a, b) => {
    if (Math.abs(a.win_pct - b.win_pct) > 0.001) {
...
function renderStandingsTable() {
  ...
  if (aggregated.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="8" class="empty-message">No data for selected seasons</td></tr>';
    return;
  }
  ...
  aggregated.forEach((franchise, index) => {
    ...
    winsCell.textContent = Math.round(franchise.wins);
    row.appendChild(winsCell);

    // Losses
    const lossesCell = document.createElement('td');
    lossesCell.textContent = Math.round(franchise.losses);
    row.appendChild(lossesCell);

    // Win %
    const winPctCell = document.createElement('td');
    winPctCell.textContent = franchise.win_pct.toFixed(3);
    ...
    maxPfCell.textContent = franchise.max_points_for.toFixed(2);
    row.appendChild(maxPfCell);

    // Accuracy
    const acCell = document.createElement('td');
    acCell.textContent = franchise.accuracy.toFixed(3);
